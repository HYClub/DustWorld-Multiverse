...
function parseContinentParam(continents, rng) {
  ...
  switch (c) {
    case '1': count = 1; radiusScale = 0.85; break;
    case '2-3': count = 2 + Math.floor(rng() * 2); radiusScale = 0.45; break;
    case '4-6': count = 4 + Math.floor(rng() * 3); radiusScale = 0.3; break;
    case 'archipelago': count = 10 + Math.floor(rng() * 10); radiusScale = 0.12; break;
    default: count = 2; radiusScale = 0.5;
  }
  ...
  for (var y = 0; y < height; y++) {
    ...
    for (var x = 0; x < width; x++) {
      ...
      if (e < threshold) {
        ...
        if (relative > 0.75) tiles[y][x] = 3;
        else if (relative > 0.4) tiles[y][x] = 2;
        else tiles[y][x] = 1;
      }
    }
  }
...
function placeRivers(tiles, elevation, threshold, rng) {
  ...
  for (var r = 0; r < riverCount; r++) {
    ...
    while (steps < maxSteps) {
      ...
      if (tiles[cy][cx] !== 4) path.push({ x: cx, y: cy });
      ...
      for (var dy = -1; dy <= 1; dy++) {
        for (var dx = -1; dx <= 1; dx++) {
          ...
          if (px >= 0 && px < width && py >= 0 && py < height) {
            if (tiles[py][px] === 0) { nx = px; ny = py; foundOcean = true; break; }
            if (elevation[py][px] < lowestE) { lowestE = elevation[py][px]; nx = px; ny = py; }
          }
        }
        if (foundOcean) break;
      }
    ...
    for (var pi = 0; pi < path.length; pi++) {
      ...
      if (p.y >= 0 && p.y < height && p.x >= 0 && p.x < width) {
        if (tiles[p.y][p.x] > 0) tiles[p.y][p.x] = 4;
      }
    }
  }
}
...
TerrainGenerator.prototype.generateResources = function (tiles, abundance, initialLife) {
  ...
  while (resources.length < baseCount && attempts < maxAttempts) {
    ...
    for (var ri = 0; ri < resources.length; ri++) {
      if (resources[ri].x === x && resources[ri].y === y) { duplicate = true; break; }
    }
    if (duplicate) continue;
    var type, amount;
    var roll = rng();
    if (tile === 3) { type = 'ore'; amount = 30 + Math.floor(rng() * 71); }
    else if (tile === 2) { type = roll < 0.6 ? 'wood' : 'food'; amount = 30 + Math.floor(rng() * 71); }
    else if (tile === 4) { type = roll < 0.5 ? 'water' : 'food'; amount = 20 + Math.floor(rng() * 61); }
    else { type = roll < 0.4 ? 'food' : (roll < 0.7 ? 'wood' : 'water'); amount = 20 + Math.floor(rng() * 81); }
...
function generateSettlementName(existingNames, rng) {
  ...
  if (available.length === 0) return 'Settlement' + Math.floor(rng() * 10000);
    ...
    for (var i = 0; i < count; i++) {
      ...
      for (var att = 0; att < 100; att++) {
        ...
        if (tiles[y][x] !== 0 && !usedPositions[key]) {
          ...
          settlements.push({ id: 'SET' + String(settlements.length + 1).padStart(3, '0'), name: name, x: x, y: y, population: 10 + Math.floor(rng() * 41), tech_level: 0 });
          found = true;
          break;
        }
      }
      ...
      for (var dy = -radius; dy <= radius && !placed; dy++) {
        for (var dx = -radius; dx <= radius && !placed; dx++) {
          ...
          if (tiles[sy][sx] !== 0 && !usedPositions[key2]) {
            ...
            settlements.push({ id: 'SET' + String(settlements.length + 1).padStart(3, '0'), name: name2, x: sx, y: sy, population: 10 + Math.floor(rng() * 41), tech_level: 0 });
            placed = true;
          }
        }
      }
  ...
  while (settlements.length < count) {
    ...
    for (var att2 = 0; att2 < 100; att2++) {
      ...
      if (tiles[ry][rx] !== 0 && !usedPositions[key3]) {
        ...
        settlements.push({ id: 'SET' + String(settlements.length + 1).padStart(3, '0'), name: name3, x: rx, y: ry, population: 10 + Math.floor(rng() * 41), tech_level: 0 });
        placed2 = true;
        break;
      }
    }
...
TerrainGenerator.prototype.setSeed = function (seed) { this.seed = seed; };
TerrainGenerator.prototype.getSeed = function () { return this.seed; };
